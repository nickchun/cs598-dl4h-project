...
#
# 5-Label ablation classification
...
def vectorize_data(lst_input):
    ...
    sent_len = sys_params['max_sent_len']
...
def read_ip_file(ip_file):
    """
    Reads the input file and returns the sentences and corresponding labels.

    Args:
    ip_file : str : Path to the input file

    Returns:
    x_data : list : List of input sentences
    y_data : list : List of corresponding labels
    """
    padd = sys_params['str_padd']
    ...
    with open(ip_file) as f:
        ...
        for row in csv_reader:
            # Check if row has at least one element
            if row:
                # print(row[0])
                sent = row[1]
                ...
                sent = filter(lambda x: x.lower() in printable, sent)

                # Remove punctuation
                lst_tokens = [item for item in list(sent) if item not in punctuations]
                ...
                y_data.append(severity_classes[row[-1].strip()])
...
def get_cnn_model():
    ...
    l_conv = Conv2D(filters=cnn_params['no_filters'], kernel_size=(cnn_params['kernels'][0], emb_dim), strides=cnn_params['c_stride'],
